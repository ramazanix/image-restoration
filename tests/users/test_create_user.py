...
async def test_create_couple_users(client: AsyncClient, create_user):
    ...
    response = await client.post(
        "/api/users", json={"username": "Tom", "password": "tom_password"}
    )
...
async def test_create_invalid_user_data(client: AsyncClient):
    ...
    """
    response = await client.post("/api/users", json={"a": "b"})
    assert response.status_code == 422

    response = await client.post("/api/users", json={"username": ""})
    assert response.status_code == 422

    response = await client.post("/api/users", json={"password": ""})
    assert response.status_code == 422

    response = await client.post("/api/users", json={"username": "", "password": ""})
    assert response.status_code == 422
...
async def test_create_user_short_username(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "u", "password": "password"}
    )
    assert response.status_code == 422
...
async def test_create_user_long_username(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "me" * 21, "password": "password"}
    )
    assert response.status_code == 422
...
async def test_create_user_short_password(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "Alex", "password": "123"}
    )
    assert response.status_code == 422
...
async def test_create_user_long_password(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "Alex", "password": "123" * 11}
    )
    assert response.status_code == 422
...
async def test_create_user_short_fields(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "u", "password": "passwd"}
    )
    assert response.status_code == 422
...
async def test_create_user_long_fields(client: AsyncClient):
    ...
    """
    response = await client.post(
        "/api/users", json={"username": "Joe" * 10, "password": "passwd" * 6}
    )
    assert response.status_code == 422
